...
class GraphRAGManager:
    ...
    def save(self,rag_pipeline):
        """保存知识库"""
        
        try:
            rag_pipeline.save("default")
...
async def rebuild_index_background(rag_pipeline):
    """后台任务：重建索引"""
    try:
        logger.info("📊 开始重建 GraphRAG 索引...")
        rag_pipeline.rebuild_index()
        graphrag_manager.save(rag_pipeline)
...
async def upload_document(
    ...
    try:
        ...
        doc_uuid = rag_pipeline.add_document(str(file_path), metadata)
        doc_info = rag_pipeline.documents[
            rag_pipeline.uuid_to_docid[doc_uuid]
        ]
        
        if auto_rebuild:
            background_tasks.add_task(rebuild_index_background, rag_pipeline)
...
async def query_knowledge_base(request: QueryRequest):
    ...
    try:
        ...
        # 执行查询
        answer = rag_pipeline.global_query(
            request.question,

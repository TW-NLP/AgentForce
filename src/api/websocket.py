...
def setup_websocket_routes(app: FastAPI):
    ...
    async def websocket_chat(websocket: WebSocket):
        ...
        try:
            ...
            if session_history and session_history.get("conversations"):
                await websocket.send_json({
                    "type": "history",
                    "data": {
                        "session_id": session_id,
                        "conversations": session_history["conversations"],
                        "created_at": session_history.get("created_at"),
                        "count": len(session_history["conversations"])
                    }
                })
    ...
    async def websocket_stream_simple(websocket: WebSocket):
        ...
        try:
            ...
            try:
                session_history = history_manager.get_session_history(session_id)
                if session_history:
                    conversations = session_history.get("conversations", [])
                    if conversations:
                        await websocket.send_json({
                            "type": "history",
                            "data": {
                        ...
                        logger.info(f"[{session_id}] 已发送 {len(conversations)} 条历史记录")
                    else:
                        logger.info(f"[{session_id}] 会话存在但无对话记录")
                else:
                    logger.info(f"[{session_id}] 新会话，无历史记录")
            except Exception as e:
                logger.error(f"[{session_id}] 加载历史记录失败: {e}")
    ...
    #         session_history = history_manager.get_session_history(session_id)
    #         if session_history and session_history.get("conversations"):  # 这行是对的
    #             await websocket.send_json({
    #                 "type": "history",
    #                 "data": {
    #                     "session_id": session_id,
    ...
    #                 }
    #             })
    #             # ❌ 问题在这里：不要在 if 里面访问可能为 None 的值
    #             logger.info(f"[{session_id}] 已发送 {len(session_history.get('conversations', []))} 条历史记录")
    #         else:
    #             logger.info(f"[{session_id}] 新会话，无历史记录")
    #         logger.info(f"[{session_id}] 已发送 {len(session_history['conversations'])} 条历史记录")
